...
CODA = $(foreach coda,$(L_CODA),$(shell printf 'sim-01/%s.RData' $(coda)))
...
CODA_COUNT = $(foreach coda,$(L_CODA_COUNT),$(shell printf 'sim-01/%s.RData' $(coda)))
...
REPLACEMENT = $(foreach replacement,$(L_REPLACEMENT),$(shell printf 'sim-01/%s.RData' $(replacement)))

L_EVALUATE = $(foreach evaluate,$(L_evaluate),$(foreach replacement,$(L_REPLACEMENT),$(shell printf 'evaluate_%s-%s' $(evaluate) $(replacement))))
EVALUATE = $(foreach evaluate,$(L_EVALUATE),$(shell printf 'sim-01/%s.RData' $(evaluate)))
...
sim-01/replacement_gbm-%.RData : sim-01/replacement_gbm.R $(CODA_COUNT)
	Rscript -e 'GEN = "$*"; source("$<")'

sim-01/replacement_czm-%.RData : sim-01/replacement_czm.R $(CODA_COUNT)
	Rscript -e 'GEN = "$*"; source("$<")'

sim-01/evaluate_stress-%.RData : sim-01/evaluate_stress.R $(REPLACEMENT)
	Rscript -e 'GEN = "$*"; source("$<")'

sim-01/evaluate_frobenius-%.RData : sim-01/evaluate_frobenius.R $(REPLACEMENT)
	Rscript -e 'GEN = "$*"; source("$<")'

sim-01/evaluate_paired.distance-%.RData : sim-01/evaluate_paired.distance.R $(REPLACEMENT)
	Rscript -e 'GEN = "$*"; source("$<")'
