...
if(!exists("GEN")) GEN = "count_uniform-size_00030-data_parliament-seed_00003"
...
build_B = function(iz, x){
  B = matrix(0, nrow = d+1, ncol = d)
  part = rep(0, d+1)
  part[x!=0] = -1
  part[iz] = +1
  B[,1] = sbp_basis(matrix(part, ncol = 1), silent = TRUE)
  B[x != 0,2:(sum(x != 0))] = Bd[[sum(x != 0)]]
  
  part = matrix(2*(x == 0 & (1:(d+1) != iz) ) - 1, ncol = 1)
  B[,d1+1] = sbp_basis(part, silent = TRUE)
  B[(x == 0 & (1:(d+1) != iz)), -(1:(d1+1))] = Bd[[sum(x == 0)-1]]
  B
}
lB = lapply(wz, build_B, x)
lBt = lapply(lB, function(B) t(B) %*% Bd[[1+d]])

lMt = lapply(lBt, function(Bt) as.vector(Bt %*% M)[1:d1])
lSt = lapply(lBt, function(Bt) (Bt %*% S %*% t(Bt))[1:d1,1:d1])

h2 = as.vector(t(Bd[[sum(x!=0)]]) %*% log(x)[x!=0])

i1 = 1
i2 = -1

lInvSt2 = mapply(function(st){
  MASS::ginv(st[i2,i2])
}, lSt, SIMPLIFY = FALSE)
lMc = mapply(function(mt, st, invst2){
  as.vector(mt[i1] + st[i1,i2] %*% invst2 %*% (h2-mt[i2]))
}, lMt, lSt, lInvSt2, SIMPLIFY = FALSE)
lSc = mapply(function(st, invst2){
  st[i1,i1] - st[i1,i2] %*% invst2 %*% st[i2,i1]
}, lSt, lInvSt2, SIMPLIFY = FALSE)

lNapprox = mapply(function(mc, sc, B){
  c_lrnm_cond_posterior_approximation_vec(x1, mc, MASS::ginv(sc), h2, B[B[,1]!=0,1:d1])
}, lMc, lSc, lB, SIMPLIFY = FALSE)

lMoments = mapply(function(napprox, mt, st, B){
  c_moments_lrnm_cond_hermite(x1, napprox[,1], napprox[1,1,drop=FALSE], mt, st, h2, B[B[,1]!=0,1:d1], mu_centering = 0, order = 10)
}, lNapprox, lMt, lSt, lB, SIMPLIFY = FALSE)

M1 = mapply(function(moments, Bt){
  c(moments[2], h2)
}, lMoments, lBt) |> t()
...
save(P.rpl, file = sprintf("sim-01/data/replacement_lrnm-cem-hermite-%s.RData", GEN))
