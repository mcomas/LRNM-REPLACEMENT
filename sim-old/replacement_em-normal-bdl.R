...
while(CONT){
  ...
  lMoments.wZ = mapply(function(i, mt, st, B){
    
    xl = X[i,]
    xl[iZ[i,]] = BL[iZ[i,]]
    bl = log(xl) %*% B
    sapply(1:sZ[i], function(i1){
      i2 = -i1
      invS2 = MASS::ginv(st[i2,i2])
      M1 = as.vector(mt[i1] + st[i1,i2] %*% invS2 %*% (bl[i2]-mt[i2]))
      S1 = st[i1,i1] - st[i1,i2] %*% invS2 %*% st[i2,i1]
      
      beta = (bl[i1] - M1) / sqrt(S1)
      b1_m1 = M1 - dnorm(beta) / pnorm(beta) * sqrt(S1)
      b1_m2 = S1 * (1 - beta  * dnorm(beta) / pnorm(beta) - (dnorm(beta)/ pnorm(beta))^2 )
      c(m1 = b1_m1, m2 = b1_m2 + b1_m1^2)
    })
  }, wZ, lMt, lSt, lB, SIMPLIFY = FALSE)
  
  M1.wZ = mapply(function(moments, h2, Bt.inv){
    Bt.inv %*% c(moments[1,], h2)
  ...
  M2.wZ = mapply(function(moments, h2, Bt.inv){
    ...
    Bt.inv %*% rbind(
      cbind(S, moments[1,] %*% t(h2)),
      cbind(h2 %*% t(moments[1,]), h2 %*% t(h2))) %*% t(Bt.inv)
